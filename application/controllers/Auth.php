    ...
    public function check_account()
    {
        ...
        if ($query === 1) {
            $this->session->set_flashdata('alert', '<p class="box-msg">
        			...
        			<b style="font-size: 20px">GAGAL</b><br>User yang Anda masukkan tidak terdaftar.</div>
        			</div>
        			</p>
            ');
        } elseif ($query === 2) {
    ...
    public function login()
    {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() && $error === true) {
                ...
                $this->template->load('authentication/layouts/template', 'authentication/loginn', $data);
            }
        } else {
            $this->template->load('authentication/layouts/template', 'authentication/loginn', $data);
        }
